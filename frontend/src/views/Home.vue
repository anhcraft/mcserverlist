                ...
                <template slot-scope="props">
                    <b-table-column field="name" label="Tên máy chủ" sortable>
                        <b-tooltip label="Nhấn vào để xem thêm về máy chủ này" size="is-small" multilined type="is-dark" position="is-left">
                            <div class="columns is-mobile is-centered is-vcentered" style="cursor:pointer" v-on:click="view(props.row)">
                                <div class="column">
                                    <img :src="props.row.query.icon" alt="Biểu tượng" style="max-width:64px"/>
                                </div>
                                <div class="column">
                                    <b :class="'has-text-' + randomColors()">{{ props.row.name }}</b>
                                </div>
                            </div>
                        </b-tooltip>
                    </b-table-column>

                    <b-table-column field="ip" label="IP">
                        <b-tooltip label="Nhấn vào để sao chép" size="is-small" multilined type="is-dark" position="is-top">
                            <b-button icon-right="content-copy" size="is-small" v-on:click="copy(props.row.ip + ':' + props.row.port)">
                                {{props.row.ip}}<span v-if="props.row.port !== 25565">:{{props.row.port}}</span>
                            </b-button>
                        </b-tooltip>
                    </b-table-column>

                    <b-table-column field="tags" label="Thẻ">
                        <b-taglist>
                    ...
                    <b-table-column field="status" label="Tình trạng">
                        <span v-if="props.row.query.online" class="has-text-success">
                            <b-icon icon="check-decagram" size="is-small"></b-icon> Online
                        </span>
                        <span class="has-text-danger" v-else><b-icon icon="alert" size="is-small"></b-icon> Offline</span>
                    </b-table-column>

                    <b-table-column field="players" label="Người chơi">
