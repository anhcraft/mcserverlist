            ...
            <div class="columns">
                <div class="column is-3">
                    ...
                    <b-tooltip label="Nhấn vào để sao chép" size="is-small" multilined type="is-dark" position="is-top">
                        <b-button icon-right="content-copy" size="is-small" v-on:click="copy(serverViewData.server.ip + ':' + serverViewData.server.port)">
                            {{serverViewData.server.ip}}<span v-if="serverViewData.server.port !== 25565">:{{serverViewData.server.port}}</span>
                        </b-button>
                    </b-tooltip>
                </div>
                ...
                <div class="column">
                    <div class="columns">
                        <div class="column is-3">
                            <img :src="serverViewData.server.query.icon" alt="Biểu tượng" style="max-width:64px"/>
                        </div>
                        <div class="column">
                            ...
                            <div v-if="serverViewData.server.website.length > 0">
                                Trang web: <a :href="serverViewData.server.website" target="_blank">{{ serverViewData.server.website }}</a>
                            </div>
                        </div>
                    </div>
                </div>
