      ...
      <div class="hero-body">
        <div class="container">
          ...
          <section class="mt-15">
            ...
            <b-button class="mt-10 m-tt-10 m-mt-10" icon-left="check" outlined type="is-info" @click="edit">Xác nhận
            </b-button>
          </section>
        </div>
      </div>
  ...
  methods: {
    loadServer() {
      this.restrictAccess(() => {
        ...
        this.lock = true;
        this.axios.get(`${this.config.server_api_url}/server/get/${this.$route.params.sid}?params=id,name,ip,port,intro,website,createdDate,creator,tags,diffPing,pingIp,pingPort`).then((res) => {
          if (res.data.code === 2) {
